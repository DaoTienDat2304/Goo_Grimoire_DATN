...
public class FarmDifficulty
{
    ...
    public int bossMagicAttack = 100;
...
public class FarmModeManager : MonoBehaviour
{
    ...
    public void RefreshDifficultyStats()
    {
        ...
        void Apply(int i, int hp, int atk, int matk, int def, int spd, float cr, float cd, int reward)
        {
            if (i >= difficulties.Count) return;
            difficulties[i].bossHP      = hp;
            difficulties[i].bossAttack  = atk;
            difficulties[i].bossMagicAttack = matk;
            difficulties[i].bossDefense = def;
            difficulties[i].bossSpeed   = spd;
            difficulties[i].bossCritRate = cr;
            difficulties[i].bossCritDMG  = cd;
            difficulties[i].rewardCoins = reward;
        }

        Apply(0, rc.FarmEasyBossHP,    rc.FarmEasyBossAttack,    rc.FarmEasyBossMagicAttack,    rc.FarmEasyBossDefense,    rc.FarmEasyBossSpeed,    0.05f, 1.30f, rc.FarmEasyReward);
        Apply(1, rc.FarmMediumBossHP,  rc.FarmMediumBossAttack,  rc.FarmMediumBossMagicAttack,  rc.FarmMediumBossDefense,  rc.FarmMediumBossSpeed,  0.06f, 1.35f, rc.FarmMediumReward);
        Apply(2, rc.FarmHardBossHP,    rc.FarmHardBossAttack,    rc.FarmHardBossMagicAttack,    rc.FarmHardBossDefense,    rc.FarmHardBossSpeed,    0.08f, 1.45f, rc.FarmHardReward);
        Apply(3, rc.FarmExtremeBossHP, rc.FarmExtremeBossAttack, rc.FarmExtremeBossMagicAttack, rc.FarmExtremeBossDefense, rc.FarmExtremeBossSpeed, 0.13f, 1.70f, rc.FarmExtremeReward);
        Apply(4, rc.FarmHellBossHP,    rc.FarmHellBossAttack,    rc.FarmHellBossMagicAttack,    rc.FarmHellBossDefense,    rc.FarmHellBossSpeed,    0.20f, 2.20f, rc.FarmHellReward);
    ...
    private void InitializeDefaultDifficulties()
    {
        ...
        FarmDifficulty Make(string name, string desc,
            int hp, int atk, int matk, int def, int spd, float cr, float cd, int reward) =>
            new FarmDifficulty
            {
                difficultyName = name,
                description    = desc,
                ...
                bossMagicAttack = rc != null ? rc.GetFarmStat(name, "magic_attack", matk) : matk,
        ...
        {
            Make("easy",    "Boss yếu, reward ít",       100,  30,  60,  20,  15,  0.05f, 1.30f, 50),
            Make("medium",  "Boss vừa, reward vừa",      200,  60,  120, 40,  25,  0.06f, 1.35f, 150),
            Make("hard",    "Boss mạnh, reward nhiều",   400,  120, 240, 80,  40,  0.08f, 1.45f, 300),
            Make("extreme", "Boss cực mạnh, reward lớn", 800,  200, 400, 150, 60,  0.13f, 1.70f, 600),
            Make("hell",    "Thử thách tối thượng",      1500, 350, 700, 250, 90,  0.20f, 2.20f, 1200),
        };
